...
> **Comparative analysis:** `cosolventAI-vs-cosolvent-beta-analysis.md`  
> **Codebase assessed:** cosolvent-beta (this repo)  
> **Date:** 2026-02-19 (preamble updated 2026-02-22)  
> **Author:** Generated by Antigravity Agent for DeeperPoint

---

## Preamble: Why This Roadmap Exists

The CosolventAI roadmap was written against a separate codebase (TypeScript/Next.js/Supabase). In parallel, cosolvent-beta was built as a Python/FastAPI backend with working implementations of many features the original roadmap described as missing. This document **rewrites the roadmap to target cosolvent-beta as the codebase going forward**, preserving the original roadmap's whitepaper coverage while:

1. Crediting what cosolvent-beta already has (no need to rebuild)
2. Identifying what needs to be **added** to reach whitepaper alignment
3. Flagging **unresolved conflicts** between the two architectural approaches — these appear inline where they arise, and are collected in [Appendix A](#appendix-a--conflict-register) for consolidated review

> **⚠️ Consolidation decision (February 22, 2026):**
> **cosolvent-beta is the sole active Cosolvent codebase going forward.** Both the original `Cosolvent` repository and the `CosolventAI` repository are **deprecated** and should be ignored for all future development. Valuable ideas from the CosolventAI roadmap have been extracted and integrated into this document. Reference copies of the deprecated roadmaps are preserved in `MarketForge/sources/` for historical context. The CosolventAI roadmap header above is retained as a provenance reference only.

---

## Executive Summary

The whitepaper defines Cosolvent as an open-source framework for thin market automation comprising eight core modules. cosolvent-beta already provides working implementations of several foundational features — marketplace configuration via YAML compiler, dynamic profile schemas, three-tier visibility, facilitator participant types, AI-assisted onboarding, vector search, admin oversight, communication, permissions, and document processing.

The remaining work falls into three categories:

| Category                 | Description                                                | Sections            |
| ------------------------ | ---------------------------------------------------------- | ------------------- |
| **Already built**        | Features cosolvent-beta has that align with the roadmap    | §1                  |
| **Extension work**       | Roadmap features to add on the existing foundation         | §2–§17              |
| **Unresolved conflicts** | Architectural choices requiring judgment before proceeding | Inline + Appendix A |
...
cosolvent-beta provides working implementations of the following. **No rebuild is needed** — these are the foundation the remaining roadmap builds on.
...
**Roadmap equivalent:** Section 21 (Slots Architecture) — cosolvent-beta has a more opinionated and auditable implementation than the original roadmap described.
...
> **⚠️ CONFLICT C5 — Frontend Strategy** applies here. cosolvent-beta has no user-facing frontend. See [Appendix A, C5](#c5-frontend-strategy).
...
> **⚠️ CONFLICT C3 — Participant Type Limit** applies here. cosolvent-beta enforces a maximum of 3 participant types. See [Appendix A, C3](#c3-participant-type-count-limit).
...
> **📌 SCOPE NOTE — Deal Negotiation Deferred:** The Deal entity is currently scoped as a **collaborative record** of deal parameters and moving parts — not an automated negotiation engine. The deal negotiation state machine (issue-level bilateral bargaining, offer/counter/accept cycles, deal-level state aggregation) described in `DealStateDiagram.mmd` is deferred pending further design consideration. The Deal entity tracks: who is involved, what is being transacted, which facilitator roles are relevant, and what context (documents, regulatory flags, AI assessments) applies. See MarketForge `ForgeRoadMap.md`, Appendix A.
...
> **📌 SCOPE NOTE:** The Handoff Artifact is assembled from the collaborative deal record — it does not require or assume an automated negotiation process. It compiles information from profiles, facilitator capabilities, shared documents, regulatory context, and AI assessments into a structured output for downstream actors.
...
- **Deal progression workflow** — inquiry → qualification → collaborative deal structuring → human approval → Handoff Artifact. *(Note: automated negotiation component deferred — see §3.1 scope note.)*
...
cosolvent-beta is hardcoded to OpenAI (`llm_client.py` directly imports `openai.AsyncOpenAI`, model list limited to three OpenAI models).
...
This phasing assumes cosolvent-beta's existing codebase and architecture. Conflict decisions (Appendix A) are noted as gating items.
...
*Builds on cosolvent-beta's existing profile and visibility infrastructure. 4–6 weeks.*
...
> **📌 SCOPE NOTE:** Items 1.9–1.11 are scoped to the Deal entity as a **collaborative record model**, not an automated negotiation state machine. The deal negotiation process (issue-level bargaining, deal-level state aggregation, agent-mediated negotiation) is deferred. See §3.1 scope note and MarketForge `ForgeRoadMap.md`, Appendix A.
...
| A1.7 | Deal Brief assembly workflow *(negotiation deferred — see §3.1)* | A1.6, 1.11 |
...
This is shorter than the original roadmap's estimate because cosolvent-beta starts with a significantly more complete foundation.
...
4. **The framework defines structure; the vertical defines content.** cosolvent-beta's YAML compiler already embodies this.
...
11. **Never destroy information through premature standardisation.** cosolvent-beta's dynamic schemas embody this.

#### D — How we implement it

12. **Design for cognitive bandwidth constraints.** Curated subsets, not raw data dumps.
13. **Prompt-driven, not code-driven.** cosolvent-beta's prompt management supports this.

---

## Appendix A — Conflict Register

The following architectural conflicts between the CosolventAI roadmap and the cosolvent-beta codebase require judgment calls before implementation proceeds. Each is referenced inline in the sections where it applies.
...
**cosolvent-beta's approach:** All domain data stored in generic JSONB document tables via `CollectionProxy`. Maximum flexibility — any field without migration — but no referential integrity, no efficient joins, no database-enforced business rules.
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **A — Hybrid**          | Keep JSONB for flexible entities (profiles, messages, notifications). Typed relational tables for relational entities (deals, transactions, disputes, trust scores). | Best of both worlds. cosolvent-beta already does this partially (`profile_vectors`, `ai_document_chunks` are typed). | Two data access patterns to maintain.                                                                   |
...
**cosolvent-beta's approach:** Configuration is a build-time artifact. YAML compiled into generated code, database migrations, and OpenAPI specs. Changes require recompilation and redeployment.
...
**Already partially resolved:** cosolvent-beta allows runtime changes for some settings — LLM parameters, prompts, admin overrides via admin API. The compiler approach only applies to structural config (participant types, schemas, communication rules).
...
**cosolvent-beta's approach:** `Maximum 3 participant types for MVP` validation rule in `MarketplaceConfig.cross_validate()`.
...
**cosolvent-beta's approach:** Hardcoded to OpenAI `text-embedding-3-small` (1536 dimensions) in `db_schema.py` (`Vector(1536)`) and `indexer.py`.
...
**cosolvent-beta's approach:** No user-facing frontend. API-only backend with operator setup panel.

**Roadmap's assumption:** User-facing frontend (the original CosolventAI had a partial Next.js/React frontend).

**Options:**

| Option                             | Description                                                                         | Pros                                          | Cons                                                                                      |
| ---------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **A — Build new frontend**         | Choose framework (Next.js, SvelteKit, etc.) and build against cosolvent-beta's API. | Clean start. API-first design.                | Significant effort. Framework choice needed.                                              |
| **B — Adapt CosolventAI frontend** | Rework the existing Next.js frontend to target cosolvent-beta's API shapes.         | Existing code to build on.                    | API shapes are completely different. Likely more work than option A.                      |
| **C — Remain headless**            | cosolvent-beta is a backend framework only. Each vertical builds its own frontend.  | Minimal framework work. Verticals control UX. | Every user-facing feature is someone else's problem. Demo requires a frontend regardless. |
...
**cosolvent-beta's approach:** No synthetic user concept. No ClientSynth integration.
