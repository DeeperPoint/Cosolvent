...
The CosolventAI roadmap was written against a separate codebase (TypeScript/Next.js/Supabase). In parallel, Cosolvent was built as a Python/FastAPI backend with working implementations of many features the original roadmap described as missing. This document **rewrites the roadmap to target Cosolvent as the codebase going forward**, preserving the original roadmap's whitepaper coverage while:
...
**Roadmap equivalent:** Section 21 (Slots Architecture) — Cosolvent has a more opinionated and auditable implementation than the original roadmap described.
...
This is shorter than the original roadmap's estimate because Cosolvent starts with a significantly more complete foundation.
...
1. **The framework defines structure; the vertical defines content.** Cosolvent's YAML compiler already embodies this.
2. **Deals need more than two parties.** Real transactions require facilitators.
3. **The platform's job is to get parties to the table, not to run the table.** V1 ends at a Handoff Artifact.

#### C — How information flows

1. **Privacy is a prerequisite.** Fewer participants = more identifiable data.
2. **Gallery is for discovery, matching is for depth.** Never conflate the two.
3. **Users own their information boundaries.** Per-document privacy must be editable.
4. **Communication is scoped, not open.** Match or deal contexts, not general messaging.
5. **Never destroy information through premature standardisation.** Cosolvent's dynamic schemas embody this.

#### D — How we implement it

1. **Design for cognitive bandwidth constraints.** Curated subsets, not raw data dumps.
2. **Prompt-driven, not code-driven.** Cosolvent's prompt management supports this.

---

## Appendix A — Conflict Register

The following architectural conflicts between the CosolventAI roadmap and the Cosolvent codebase require judgment calls before implementation proceeds. Each is referenced inline in the sections where it applies.
...
| **B — Adapt CosolventAI frontend** | Rework the existing Next.js frontend to target Cosolvent's API shapes.         | Existing code to build on.                    | API shapes are completely different. Likely more work than option A.                      |
